...
// Pipedrive
const getAndLoadProduct = async _ => {
  const response = await fetch('https://immocitiz.pipedrive.com/api/v1/products/' + new URLSearchParams(window.location.search).get('id') + '?api_token=' + new URLSearchParams(window.location.search).get('p'));
